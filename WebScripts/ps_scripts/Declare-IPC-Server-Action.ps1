function Start-IpcPipeServer {
    param(
        [string]$PipeName = ((Get-TvsConfig).names.ipcPipeName),
        [string]$MutexName = ((Get-TvsConfig).names.ipcMutexName)
    )
    ...
    try {
        ...
        Write-VerboseDebug -Timestamp (Get-Date) -Title "IPC" -Message "Requested voice not found, using default." -ForegroundColor "Yellow"
    }
...
"@
    ...
    function Handle-Command {
        ...
        switch ($verb) {
            "SPEAK" { if ($payload) { $ttsEngine.SpeakAsync($payload) | Out-Null } }
            ...
            "SHOW_PROCESS" {
                ...
                $processesDiff = $updatedProcesses | ForEach-Object {
                    ...
                    if ($initialProcess) {
                        ...
                        if ($diff -gt 0) {
                            [PSCustomObject]@{ Name = $currentProcess.Name; Id = $currentProcess.Id; OldCPUTime = $initialProcess.CPU; NewCPUTime = $currentProcess.CPU; Difference = $diff }
                        }
                    } else {
                ...
                $processesDiff | Sort-Object -Property Difference -Descending | Format-Table -Property Name, Id, OldCPUTime, NewCPUTime, Difference -AutoSize | Out-String | Write-Host
            }
    ...
    if (-not $isNew) {
        Write-VerboseDebug -Timestamp (Get-Date) -Title "IPC" -Message "Another IPC server instance is running." -ForegroundColor "Red"
    ...
    while ($global:IPC_ContinueServer) {
        ...
        try {
            while ($pipeServer.IsConnected) {
                ...
                if ($command) {
                    Handle-Command -command $command -writer $writer -ttsEngine $tts -initialProcesses $initialProcesses
                }
            }
        } finally {
