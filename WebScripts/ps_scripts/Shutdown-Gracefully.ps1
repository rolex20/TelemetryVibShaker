...
try {
    # Use .NET WriteAllText for PowerShell 5.1 compatibility and UTF-8 without BOM.
    [System.IO.File]::WriteAllText($tempCommandPath, $payloadJson, (New-Object System.Text.UTF8Encoding($false)))
    if (-not (Test-Path -LiteralPath $tempCommandPath -PathType Leaf)) {
        throw "Failed to create temp command file: $tempCommandPath"
    }
    ...
    Write-Host "Failed to send EXIT_WATCHER: $($_.Exception.Message)" -ForegroundColor Yellow
    $continueAnyway = Read-Host 'Failed to signal watcher for graceful exit. Proceed to shutdown anyway? (Y/N)'
    if ($continueAnyway -notmatch '^[Yy]$') {
        Write-Host 'Shutdown cancelled by user.'
        exit 1
    }

    Write-Host 'Proceeding to shutdown immediately after user confirmation.'
}
...
if ($signalSucceeded) {
    ...
    # Signal phase failed and user confirmed; skip the full graceful wait and continue immediately.
    Write-Host 'Skipping graceful-wait loop because signal delivery failed.'
}
