...
function Stop-GameRuntimeTracker {
    ...
    if ($runtimeInfo.ProcessObject) {
        try {
            # Process objects cache metrics; refresh to read final CPU counters at stop time.
            $runtimeInfo.ProcessObject.Refresh()
            $cpuEndSeconds = $runtimeInfo.ProcessObject.TotalProcessorTime.TotalSeconds

            # Fallback in case the cached object cannot report a final value after exit.
            if ($cpuEndSeconds -le 0) {
                $p = Get-Process -Id $ProcessId -ErrorAction SilentlyContinue
                if ($p) {
                    $cpuEndSeconds = $p.TotalProcessorTime.TotalSeconds
                }
            }
