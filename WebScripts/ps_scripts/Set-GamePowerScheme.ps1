...
function Stop-GameRuntimeTracker {
    ...
    if ($runtimeInfo.ProcessObject) {
        try {
            ...
            $cpuEndSeconds = $runtimeInfo.ProcessObject.TotalProcessorTime.TotalSeconds
            $cpuTotalSeconds = [Math]::Max(0, ($cpuEndSeconds - $runtimeInfo.CpuStartSeconds))
        }
        catch {
            try {
                $fallbackProcess = Get-Process -Id $ProcessId -ErrorAction Stop
                $cpuEndSeconds = $fallbackProcess.TotalProcessorTime.TotalSeconds
                $cpuTotalSeconds = [Math]::Max(0, ($cpuEndSeconds - $runtimeInfo.CpuStartSeconds))
            }
            catch {
                $cpuTotalSeconds = -1.0
                Write-VerboseDebug -Timestamp (Get-Date) -Title "PLAYTIME" -ForegroundColor "DarkYellow" -Message "Could not read CPU elapsed total for $ProgramName [PID:$ProcessId]. Reporting -1."
            }
        }
        finally {
    ...
    switch ($traceName) {
        ...
        "Win32_ProcessStopTrace" { 
            ...
            if ($runtimeSummary) {
                ...
                $cpuFormatted = if ($runtimeSummary.CpuTotalSeconds -lt 0) { "-1" } else { Format-GameplayDurationText -TotalSeconds $runtimeSummary.CpuTotalSeconds }
