...
| **`parameters.eco_qos`** | Boolean | `null` | `true`, `false` | Enables Windows 10/11 **EcoQoS** (Power Throttling / Efficiency Mode).<br>• `true`: Invokes `SetProcessInformation(ProcessPowerThrottling)` on the process container (ensuring all child threads created by the process inherit EcoQoS) and calls `SetThreadInformation` on all existing active threads. Windows dynamically biases execution to E-cores, caps CPU frequency, and drops I/O and memory priorities.<br>• `false`: Explicitly clears EcoQoS on the process and threads.<br>• `null` / omitted: Leaves the process power throttling state untouched.<br>*(Requires `SeDebugPrivilege` for cross-user processes like `TiWorker`, which the engine enables automatically).* |
...
| :--- | :--- | :--- | :--- |
| **`action-per-process-boost1.json`** | **Standard Hybrid Balance** | Games use `thread_ideal_processor: "P-Cores"` and `thread_priority: "AboveNormal"` with `thread_cpu_sets: "DoNotChange"`, allowing the OS scheduler burst flexibility while steering the top active threads to P-cores. Support tools (`steamwebhelper`) throttled with `eco_qos: true` and `Idle` priority. | `FlightSimulator`, `aces` (War Thunder), `dcs`, `Ace7Game`, `forzamotorsport7`, `SmartersIPTV`, `sfvip player` |
| **`action-per-process-boost2.json`** | **CPU Sets Preference** | Sets `thread_cpu_sets: "P-Cores"` on sim processes (`FlightSimulator`, `aces`) for soft CPU set reservation. Support tools (`steamwebhelper`) throttled with `eco_qos: true` and `Idle` priority. Includes profile for `GRW` (Ghost Recon Wildlands) with 1200s delayed boost. | `FlightSimulator`, `aces`, `dcs`, `GRW` |
| **`action-per-process-boost3.json`** | **Aggressive P-Core Affinity** | Enforces hard `process_affinity: "P-Cores"` on games and VR runtimes (`OVRServer_x64`) to completely isolate the simulation workload from E-cores. Support tools (`steamwebhelper`) throttled with `eco_qos: true` and `Idle` priority. | `FlightSimulator`, `aces`, `dcs`, `acs`, `SGWContracts2` |
| **`action-per-process-boost4.json`** | **Background Throttle** | Throttles resource-heavy Windows update/telemetry processes permanently via `eco_qos: true` (Efficiency Mode) and `Idle` priority in a consolidated multi-process rule (`["TiWorker", "CompatTelRunner"]`) to prevent micro-stutters and thermal spikes without hard affinity constraints. | `TiWorker`, `CompatTelRunner` |
