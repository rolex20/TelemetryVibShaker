$scriptRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$psScriptsRoot = Split-Path -Parent $scriptRoot

$libraryScripts = @(
    'Write-VerboseDebug.ps1',
    'Get-TvsConfig.ps1',
    'Gaming-Programs.ps1',
    'Check-Admin-Privileges.ps1',
    'SetAffinityAndPriority.ps1',
    'Set-PowerScheme.ps1',
    'Set-IdealProcessor.ps1',
    'Cpu-Snapshots.ps1',
    'Set-GamePowerScheme.ps1',
    'Set-ForegroundProcess.ps1',
    'Set-Minimize.ps1',
    'Set-Maximize.ps1',
    'Set-WindowsPosition.ps1',
    'Get-WindowLocation.ps1',
    'Send-MessageViaPipe.ps1',
    'Show-CPU-Time-PerProcess.ps1',
    'Get-RenamesWatcher.ps1',
    'Get-ProcessWatcher.ps1',
    'Process-CommandFromJson.ps1',
    'Watchdog-Operations.ps1',
    'WT_MissionType1.ps1',
    'Send-IPC-ExitCommand.ps1',
    'Declare-IPC-Server-Action.ps1',
    'Monitor-War-Thunder-Distance-Multiplier.ps1'
)

foreach ($scriptName in $libraryScripts) {
    $scriptPath = Join-Path $psScriptsRoot $scriptName
    . $scriptPath
}

Export-ModuleMember -Function @(
    'Write-VerboseDebug','Get-TvsConfig','Get-GamingPrograms','Check-Admin-Privileges','SetAffinityAndPriority',
    'Get-RenamesWatcher','Get-ModificationWatcher','Get-ProcessWatchers','Create-ProcessWatcherQuery',
    'Process-CommandFromJson','ScheduleWatchdogCheck','Watchdog_Operations','Generate_WT_Mission_Type1',
    'Set-GamePowerScheme','Send-IPC-ExitCommand','Start-IpcPipeServer','Start-WarThunderDistanceMultiplierMonitor',
    'Set-ForegroundProcess','Set-Minimize','Set-Maximize','Set-WindowPosition','Get-WindowLocation',
    'Send-MessageViaPipe','Run-Actions-Per-Game','Restore-Processes-To-Default','Show-Process-Thread-Times'
)
